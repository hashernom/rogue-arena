...
function testBitmaskLogic() {
  ...
  const collideProjectilePlayer =
    (projectileMembership & playerFilter) !== 0 && (playerMembership & projectileFilter) !== 0;
  ...
  console.log(
    `   Interacción PROJECTILE->PLAYER: ${collideProjectilePlayer ? 'COLISIONA (ERROR)' : 'NO COLISIONA (OK)'}`
  );
  ...
  const collideEnemyEnemy =
    (enemyMembership & enemyFilter) !== 0 && (enemyMembership & enemyFilter) !== 0;
  ...
  console.log(
    `   Interacción ENEMY->ENEMY: ${collideEnemyEnemy ? 'COLISIONA (ERROR)' : 'NO COLISIONA (OK)'}`
  );
  ...
  const collidePickupPlayer =
    (pickupMembership & playerFilter) !== 0 && (playerMembership & pickupFilter) !== 0;
  // Interacción PICKUP vs ENEMY
  const collidePickupEnemy =
    (pickupMembership & enemyFilter) !== 0 && (enemyMembership & pickupFilter) !== 0;
  ...
  const collidePickupWall =
    (pickupMembership & wallFilter) !== 0 && (wallMembership & pickupFilter) !== 0;
  ...
  console.log(
    `   Interacción PICKUP->PLAYER: ${collidePickupPlayer ? 'COLISIONA (OK)' : 'NO COLISIONA (ERROR)'}`
  );
  console.log(`   Resultado esperado: COLISIONA -> ${collidePickupPlayer ? '✓' : '✗'}`);
  console.log(
    `   Interacción PICKUP->ENEMY: ${collidePickupEnemy ? 'COLISIONA (ERROR)' : 'NO COLISIONA (OK)'}`
  );
  console.log(`   Resultado esperado: NO COLISIONA -> ${!collidePickupEnemy ? '✓' : '✗'}`);
  console.log(
    `   Interacción PICKUP->WALL: ${collidePickupWall ? 'COLISIONA (ERROR)' : 'NO COLISIONA (OK)'}`
  );
